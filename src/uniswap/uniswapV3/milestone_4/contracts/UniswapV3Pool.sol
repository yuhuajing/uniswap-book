...
contract UniswapV3Pool is IUniswapV3Pool {
    ...
    function flash(
        ...
        if (amount0 > 0) ERC20(token0).transfer(msg.sender, amount0);
        if (amount1 > 0) ERC20(token1).transfer(msg.sender, amount1);

        IUniswapV3FlashCallback(msg.sender).uniswapV3FlashCallback(data);

        require(ERC20(token0).balanceOf(address(this)) >= balance0Before);
        require(ERC20(token1).balanceOf(address(this)) >= balance1Before);
    ...
    function balance0() internal view returns (uint256 balance) {
        balance = ERC20(token0).balanceOf(address(this));
    }
